...
def python(inp, input=None):
    #if input.host.lower() in ["30AC6C9.FD302414.F0378F09.IP","botters/doge","gateway/web/b0rked.me/ip.127.0.0.69", "dukdukd.uk","2001:41d0:8:991a:0:0:0:1","i.am.rly.sx","-"]:
    if input.nick == 'nathan':
...
def pysandbox(msg=None):
    if not msg: return "No Code"
        ...
        output = "import math, cmath, random, requests;"
        output+= "safe_list = {};"
        ...
        output+= "execdict = {'__builtins__': safe_list,'math': math,'cmath': cmath,'random': random, 'requests': requests};"
        output+= '\ntry:\n\texec(\"\"\"'+msg.replace("\"","'")+'\"\"\");\n'
