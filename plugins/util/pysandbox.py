...
def sandbox(inp=None):
    msg = inp
    if not msg: sys.exit("No Code")
    else:
        if ".__" in msg: sys.exit("Nope")
        if "['__" in msg and "__']" in msg or "[\"__" in msg and "__\"]" in msg: sys.exit("Nope")
        ...
        output+= "msg = "+msg
        output+= "exec('def foo(): '+(msg.encode(\"hex\")).decode('hex')+';\nresp=foo();\nif resp!=None: return(resp);',execdict);"
        print output
